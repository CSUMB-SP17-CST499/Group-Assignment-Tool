...
class SlackRequest():
        ...
        try:
            ...
            for i in range (0, size):
                if group == data['usergroups'][i]['name']:#if the passed in group is equal to a group found in the api request
                    users_names = self.get_user_names(data['usergroups'][i]['users'])
                    break
    ...
    def update_usergroup_users(self, user_group_id, user_ids):
    ...
    def get_usergroup_users_list(self, usergroup_id):
